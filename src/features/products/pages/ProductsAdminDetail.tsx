...
import { ProductCategoriesCard, ProductImageCard, ProductPriceCard, ProductStockCard, ProductIngredientsCard } from '../components/ProductsDetailCards';
...
const ProductsAdminDetail = () => {
    ...
    const handleAvailability = (availability: boolean) => {
        changeStockAvailable({ id: prodId, is_available: availability }, {
            onSuccess: () => {
                notifications.show({ color: "green", message: "Disponibilidad actualizada" })
            },
            onError: (error) => {
    ...
    return (
        <>
            <div className="w-full flex justify-start items-center px-4" >
                <Button onClick={() => navigate("/admin/products")} size="lg" variant="subtle" color="cyan" leftSection={<CircleArrowLeftIcon size={28} />}>Volver</Button>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-4">
                <section className="col-span-1 md:col-span-2 card flex items-center justify-between border border-solid border-zinc-500 rounded-2xl bg-slate-100 p-4">
                    <div className="">
                        ...
                        <Title order={2} style={{ marginBottom: "8px" }}>{product.name}</Title>
                        <Text size="md" >{product.description}</Text>
                    </div>
                    <button onClick={() => handleAvailability(!product.available)}>
                        <Badge color={product.available ? "teal" : "red"} variant="dot" size="lg" style={{ cursor: "pointer" }} >
                ...
                <section className="col-span-1 md:col-span-2 grid grid-cols-3 gap-4">
                    ...
                    <div className="metric-card flex flex-col gap-2 p-4 border border-solid border-zinc-500 rounded-2xl bg-slate-100">
                        ...
                        <p className="text-2xl font-medium">{product.categories?.length ?? 0}</p>
                    </div>
                </section>
                ...
                <section className="col-span-1 md:col-span-2 card">
                    <div className="metric-card flex flex-col gap-2 p-4 border border-solid border-zinc-500 rounded-2xl bg-slate-100">
                        ...
                        <div className="flex justify-between text-sm">
                            <Text>Creado</Text>
                            <span>{new Date(product.created_at).toLocaleString("es-AR")}</span>
                        </div>
                        <Divider className="mx-2" />
                        <div className="flex justify-between text-sm">
                            <Text>Actualizado</Text>
                            <span>{product.updated_at
                        ...
                        <div className="flex justify-between text-sm">
                            <Text >Eliminado</Text>
                            <Badge color={product.deleted_at ? "red" : "teal"} variant="dot" size="lg">
                                {product.deleted_at ? new Date(product.deleted_at).toLocaleString("es-AR") : "No"}
                            </Badge>
                        </div>
                    </div>
                </section>
